...
class ProcessInvoice implements ObserverInterface
{
     ...
     * @param \Vnecoms\VendorsSales\Model\Order\Email\Sender\InvoiceSender $invoiceSender
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        foreach ($vendorInvoiceItems as $vendorId => $items) {
            ...
            $shippingTaxAmount = 0;
            $discoutShippingAmount = 0;
            $baseDiscoutShippingAmount = 0;
            ...
            if ($includeShippingTax) {
                ...
                $discoutShippingAmount = $vendorOrder->getShippingDiscountAmount();
                $baseDiscoutShippingAmount = $vendorOrder->getBaseShippingDiscountAmount();
            ...
            $invoiceData = [
                ...
                'discount_description' => $vendorOrder->getDiscountDescription(),
            ...
            foreach ($items as $item) {
                ...
                $invoiceData['discount_tax_compensation_amount'] += $item->getData('discount_tax_compensation_amount');
                $invoiceData['base_discount_tax_compensation_amount'] += $item->getData('base_discount_tax_compensation_amount');

                $invoiceData['shipping_discount_tax_compensation_amount'] += $item->getData('shipping_discount_tax_compensation_amount');
                $invoiceData['base_shipping_discount_tax_compensation_amnt'] += $item->getData('base_shipping_discount_tax_compensation_amnt');
            }

            $invoiceData['discount_amount'] += $discoutShippingAmount;
            $invoiceData['base_discount_amount'] += $baseDiscoutShippingAmount;

            $invoiceData['grand_total'] = $invoiceData['subtotal'] +
                $invoiceData['shipping_amount'] +
                $invoiceData['tax_amount'] -
                abs($invoiceData['discount_amount']) +
                $invoiceData['discount_tax_compensation_amount'];

            $invoiceData['base_grand_total'] = $invoiceData['base_subtotal'] +
                $invoiceData['base_shipping_amount'] +
                $invoiceData['base_tax_amount'] -
                abs($invoiceData['base_discount_amount']) +
                $invoiceData['base_discount_tax_compensation_amount'];
            ...
            if ($invoice->getState() == Invoice::STATE_PAID) {
                ...
                $vendorOrder->setDiscountTaxCompensationInvoiced($vendorOrder->getDiscountTaxCompensationInvoiced()+ $vendorInvoice->getDiscountTaxCompensationAmount());
                $vendorOrder->setBaseDiscountTaxCompensationInvoiced($vendorOrder->getBaseDiscountTaxCompensationInvoiced()+$vendorInvoice->getBaseDiscountTaxCompensationAmount());

            }

            $currentState = $vendorOrder->getState();

            if ($currentState === Order::STATE_PROCESSING &&
                !$vendorOrder->isCanceled() &&
                !$vendorOrder->canUnhold() &&
                !$vendorOrder->canInvoice() &&
                !$vendorOrder->canShip()
            ) {
                $vendorOrder->setState(Order::STATE_COMPLETE);
            } else {
                $vendorOrder->setState(Order::STATE_PROCESSING);
            }

            $vendorOrder->setStatus($this->statusResolver->getOrderStatusByState($order, $vendorOrder->getState()));
